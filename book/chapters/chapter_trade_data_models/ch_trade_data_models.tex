...
Intuitively one would think that one would be well served by an improved forecast of the daily volume. Very little academic research exists on daily volume prediction
...
While clearly more sophisticated approaches are used, which fit more in the HFT alpha signal research we will touch in the next section,having a stable measure that is used in a consistent fashion across the various concerns is highly recommended and leads to a much more stable behavior and improved decision making.

\subsubsection{Intraday Return Normalization: Mid Quote Volatility}
While Characteristic Time is helping answer the question of 'How Long' another common concern is 'How Far' meaning how much can the stock move over a certain amount of time. As before, HFT alpha signals are better suited to have a good forecast for this but having a stable average measure is still very valuable in practice. This is a dispersion measure and the lazy 
choice could be use daily volatility scale down to the right time scale but this is a very poor choice. A better choice, and one that is often used in practice is to look at the average number of ticks that the stock will move over a certain number of characteristic times.

%TODO Show some examples


\subsubsection{Microstructure Normalization}
One common mistake of naive trading strategies is trading in a conspicuous and predictable way. Some HFT strategies are trained to spot the presence of large institutional orders so that they can trade ahead of them to build out their inventory they can then sell(buy) back at higher(lower) prices. To some extent since an execution strategy has to trade it's impossible to always completely invisible to the watchful eye of such strategies however with some modest care one can minimize such impact. A common impact minimization approach could be termed: "hide within the crowd". Using this approach trading decisions are made so as to minimize the change in short term distribution of the most common microstructure variables. Here is a brief overview
\begin{itemize}
\item Average Number Of trades: Often used to control how often to trade. Often at odds with the next measure: how much to trade.
\item Average Trade Size: A large print on exchange or a larger than average block in a dark venue would be spotted and looking at the Level III data one can guess the trade direction giving our strategy away. That being said a constant, and unusual trickle of 100 shares trades is likely to have worse impact that one large trade, in particular if that is spread far apart from other such trades. This measure, as implied above, should be different for different venue types (lit venues, gray venues, dark venues,etc.)
\item Average Quote size: When the decision to trade aggressively is made one needs to size the order appropriately. The latency between market data and decision makes it impossible to ensure that the quote size one is "attacking" will still be there when the order hits the exchange. Often some average quote size is used to help sizing.
\item Average Spread: When trading aggressively one should be sure the price at the far size is not too far away. If the current spread is much larger than average it is probably best to wait when the aggressive price is more within "normal" reach. Conversely if the spread is smaller than everage, that might mean a short term opportunity to same some "spread cost"
\end{itemize}
...
The decision of the numbers of profile bins is a trade-off between required precision and granularity during the most active periods of the day and the lack of stability due to lack of data/change in each bin. 1 minute bin is the most common choice in the industry.
...
Special days, as previously discussed, present additional challenges due to the limited number of datapoints available (e.g triple witching occurs only 4 time a year). So using larger clusters (one cluster as a limit) is often the only option. In order to maintain some of the  cluster characteristics a common approach is to use a "difference" profile created by a normalized per-bin ratio between the regular day profile and the special day profile. This gives a profile that capture the special day systematic shift. This difference profile can then been applied to the cluster profile.

\subparagraph{Dynamic Profiles}
Volume distributions are relatively stable for reasons we have discussed above but they are subject to large distortion due to some 'surprise' event such as unexpected news. In such cases the volume can spike dramatically and the overall realized distribution will end up being highly skewed.  This distortion is likely the single most common reason why the execution coverage desks get angry calls from client because their realize VWAP slippage was so poor. This is because usually this distortion is accompanied by a price dislocation and a standard VWAP strategy trading against a static profile would most often under trade that even leading too poor VWAP slippage. To address that one can build more sophisticated dynamic profiles models that take into account the autocorrelation of volume during surprise events skewing the rest of the day's profile more towards the event. Essentially no academic literature exist for for such models.

\section{Remainder of the Day Volume}
%TODO
\section{Auction Volumes}
%TODO
