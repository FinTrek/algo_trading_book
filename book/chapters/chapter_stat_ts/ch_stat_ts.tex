...
For the CAPM model \eqref{eqn:rt_minus_rf_is}, it is expected that $\alpha= 0$ if the model holds. Any deviation from the null hypothesis $H_0: \alpha= 0$ indicates the stock (alpha) performance after adjusting for the risk-return relationship. Note that $\alpha$ can be written as,
	...
	\end{equation}
When $\beta>1$, the stock is taken to be riskier than the market. If the stock is closely aligned with the market, that is $\beta=1$, $\alpha= E(r_t) - E(r_{mt})$ and thus a positive `$\alpha$' would indicate performance to market performance.
...
\begin{itemize}
\item Normalize the prices of every stock to equal unity, at the end of each calendar month, m; use the preceding twelve months as the estimation period; let $T_m$ be the number of trading days in the 12-month estimation period.
