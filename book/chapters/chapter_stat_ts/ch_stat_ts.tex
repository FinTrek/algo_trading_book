...
A criterion that is attributed to Kelly (1956)~\cite{kelly56} that was introduced in the context of gambling determines the fraction of investment capital to bet, `$f$', with the initial investment, $X_0$ and `$M$' trials of investment opportunities is based on the wealth accumulation as
	\begin{equation}\label{eqn:xmx01f}
	X_M= X_0(1+fW)^{pn} (1-fL)^{qn}
	\end{equation}
...
where `$\gamma$' is the risk aversion parameter, with the budget constraint,
	...
	\dfrac{dX_t}{r_t}= r_f \; dt + w_t (\mu_0 + \mu_1 r_t - r_f) \; dt + w_t \sigma \; dB_t.
	\end{equation}
...
which is the Sharpe ratio adjusted for the risk-aversion. If the information from the state variable, although uncertain, is considered then the optimal investment size is given as
	...
	w(\gamma)= \dfrac{\mu - r_f}{\gamma \cdot \sigma^2 - (1-\gamma)(f)}
	\end{equation}
where `$f$' depends `$\mu_1$' in (\ref{eqn:dxtxt}) that indicates its influence of state variable on the return process and volatility in the state variable; for details see Zhu and Zhou (2009)~\cite{zhuzhou09}. As this quantity `$f$' can take a range of values, the adjustment factor $(1-\gamma)f$ can take positive or negative values. Note when $\gamma=1$, $w(\gamma)=w$, the standard measure of Sharpe ratio. 
