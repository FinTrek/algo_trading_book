...
\noindent Buy order defined by: $BUY_{price}, BUY_{size}$ \todo{Caleb: how do I ensure the second 'BUY' doesn't show up as italic?}

\noindent Sell order defined by: $SELL_{price}, SELL_{size}$ \todo{Caleb: how do I ensure the second 'SELL' doesn't show up as italic?}


\noindent Prevailing midpoint defined as: MID \\

\noindent I$f BUY_{price} >= MID:$ \todo{Caleb: how do I ensure the 'MID' doesn't show up as italic?}

 
If $SELL_{price}$ <= MID: \todo{Caleb: how do I ensure the 'MID' doesn't show up as italic?}

      Executed order quantity = $min(SELL_{size}, BUY_{size})$ \todo{Caleb: how do I ensure the second 'BUY' doesn't show up as italic?}\todo{Caleb: how do I create an extra indent toward the right?}


      Executed order price = MID\todo{Caleb: how do I create an extra indent toward the right aligned with the one above?} \\
