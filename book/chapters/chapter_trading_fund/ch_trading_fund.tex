...
% Electronification and the Start of Fragmentation:
...
% The Birth of High Frequency Trading and Algorithmic Trading:
...
% Long-Short Asset Managers and Hedge Funds:
...
% Broker-Dealers (a.k.a. Sell Side):
...
% HFTs and ELPs (Electronic Liquidity Providers), DMMs (Designated Market Makers):
...
% Single Dealer Platform/Systematic Internalizers:
...
Orders on the buy side are called ``bids'' while those on the sell side are called ``asks''. The above events are illustrated in Figure~\ref{fig:limbk1} to Figure~\ref{fig:limbk3}. When a market (or marketable) order is submitted, it decreases the number of outstanding orders at the opposite best price. For example, if a market bid order arrives, it will decrease the number of outstanding asks at the best price. All unexecuted limit orders can be canceled. When a cancellation occurs, it will decrease the number of outstanding orders at the specified price level.
	...
	\def\borderwidth{0.7pt}
	\def\barthick{0.5pt}
	\def\barwidth{0.4/2}
	
	% Axes Label
	\node at (-1.4,3.2) {\rotatebox{90}{volume}};
	\node at (5.0,-1) {price};
	
	% Border
	\draw[line width=\borderwidth] (-0.4,0) -- (-0.4,6.8) -- (10.2,6.8) -- (10.2,0) -- (-0.4,0);	
	
	% X Ticks
	\foreach \x in {0,...,7} 
	\draw (0+0.7*2*\x,0) -- (0+0.7*2*\x,-0.2);
	\node at (0,-0.4) {73600};
	\node at (1.4,-0.4) {73800};	
	\node at (2.8,-0.4) {74000};	
	\node at (4.2,-0.4) {74200};	
	\node at (5.6,-0.4) {74400};	
	\node at (7.0,-0.4) {74600};
	\node at (8.4,-0.4) {74800};
	\node at (9.8,-0.4) {75000};	
	
	% Y Ticks
	\foreach \x in {0,...,6}
		\draw (-0.4,\x+0.2) -- (-0.4-0.2,\x+0.2);
	\node at (-0.8,0.2) {0};
	\node at (-0.8,1.2) {10};
	\node at (-0.8,2.2) {20};
	\node at (-0.8,3.2) {30};
	...
	% Corner Label
	\draw (-0.4,5.6) -- (1.3,5.6) -- (1.3,6.8);
	
	\node at (0.7,6.5) {bids};
	\draw[fill=mypurple] (-0.1,6.3) rectangle (0.2,6.6);
	\node at (0.7,6.0) {asks};
	\draw[fill=mypink] (-0.1,5.8) rectangle (0.2,6.1);
	
	% Bars
	\draw[line width=\barthick,fill=mypurple] (0-\barwidth,0.1) rectangle (0+\barwidth,0.55);
	\draw[line width=\barthick,fill=mypurple] (0.7-\barwidth,0.1) rectangle (0.7+\barwidth,2.4);
	\draw[line width=\barthick,fill=mypurple] (1.4-\barwidth,0.1) rectangle (1.4+\barwidth,1.4);
	\draw[line width=\barthick,fill=mypurple] (2.1-\barwidth,0.1) rectangle (2.1+\barwidth,5.0);
	\draw[line width=\barthick,fill=mygray] (2.1-\barwidth,3.0) rectangle (2.1+\barwidth,5.0);
	\draw[line width=\barthick,fill=mypurple] (2.8-\barwidth,0.1) rectangle (2.8+\barwidth,0.65);
	
	\draw[line width=\barthick,fill=mypink] (7.0-\barwidth,0.1) rectangle (7.0+\barwidth,2.0);
	\draw[line width=\barthick,fill=mypink] (7.7-\barwidth,0.1) rectangle (7.7+\barwidth,6.7);
	\draw[line width=\barthick,fill=mypink] (8.4-\barwidth,0.1) rectangle (8.4+\barwidth,3.8);
	\draw[line width=\barthick,fill=mypink] (9.1-\barwidth,0.1) rectangle (9.1+\barwidth,0.65);
	\draw[line width=\barthick,fill=mypink] (9.8-\barwidth,0.1) rectangle (9.8+\barwidth,4.0);
	
	% Bracket
	\draw[snake=brace,line width=0.75pt,mirror snake] (2.5,3) -- ++(0,2);
	\node at (3.3,4) {limit bid};
	\end{tikzpicture}
	...
	\draw[line width=\barthick,fill=mypink] (7.0-\barwidth,0.1) rectangle (7.0+\barwidth,2.0);
	\draw[line width=\barthick,fill=mygray] (7.0-\barwidth,0.8) rectangle (7.0+\barwidth,2.0);
	\draw[line width=\barthick,fill=mypink] (7.7-\barwidth,0.1) rectangle (7.7+\barwidth,6.7);
	\draw[line width=\barthick,fill=mypink] (8.4-\barwidth,0.1) rectangle (8.4+\barwidth,3.8);
	\draw[line width=\barthick,fill=mypink] (9.1-\barwidth,0.1) rectangle (9.1+\barwidth,0.65);
	\draw[line width=\barthick,fill=mypink] (9.8-\barwidth,0.1) rectangle (9.8+\barwidth,4.0);
	...
	% Bracket
	\draw[snake=brace,line width=0.75pt] (6.6,0.8) -- ++(0,1.2);
	\node at (5.6,1.4) {market bid};
	\end{tikzpicture}
	...
	\draw[line width=\barthick,fill=mypink] (7.0-\barwidth,0.1) rectangle (7.0+\barwidth,2.0);
	\draw[line width=\barthick,fill=mypink] (7.7-\barwidth,0.1) rectangle (7.7+\barwidth,6.7);
	\draw[line width=\barthick,fill=mygray] (7.7-\barwidth,5.2) rectangle (7.7+\barwidth,6.7);
	\draw[line width=\barthick,fill=mypink] (8.4-\barwidth,0.1) rectangle (8.4+\barwidth,3.8);
	\draw[line width=\barthick,fill=mypink] (9.1-\barwidth,0.1) rectangle (9.1+\barwidth,0.65);
	\draw[line width=\barthick,fill=mypink] (9.8-\barwidth,0.1) rectangle (9.8+\barwidth,4.0);
	
	% Bracket
	\draw[snake=brace,line width=0.75pt] (7.3,5.2) -- ++(0,1.5);
  	\node at (6.0,6.0) {ask cancellation};
	\end{tikzpicture} 
...
\section{Taxonomy of Data Used in Algorithmic Trading\label{sec:taxonomy_data_algtrad}}
...
% Reference Data
\subsection{Reference Data\label{sec:reference_data}} 
...
\subsection{Market Data Derived Statistics\label{sec:market_data_derived}}
...
\emph{``The market for a stock is liquid if the following conditions hold}:
	\begin{itemize}
	\item {\itshape There are always bid and ask prices for the investor who wants to buy or sell small amounts of stock immediately.}
	\item {\itshape The difference between the bid and ask prices (the spread) is always small.}
	\item {\itshape An investor who is buying or selling a large amount of stock, in the absence of special information, can expect to do so over a long period of time at a price not very different, on average, from the current market price.}
	\item {\itshape An investor can buy or sell a large block of stock immediately, but at a premium or discount that depends on the size of the block. The larger the block, the larger the premium or discount.''}
	\end{itemize}
...
\item[\textbf{d)}] {\bfseries The objectives of the modern Market Maker:}
