...
\item \textbf{Exchange Specific Data:} Exchange sessions hours, lunch break, holidays, tick tables, trade and quote lots, DST adjustments to gmt (eg significant impact on intraday liquidity patterns in Brazil), limit up or down constraints and short sell constraints, disrupted days (exchange outage or disruption, market data issues, ...)
...
For instance, in order to assess accessible liquidity for a trading algorithm, trades that are published for reporting purposes (e.g. negotiated transactions that happened off-exchange) need to be excluded. Execution algorithms also leverage extensively intraday liquidity distribution metrics to gauge their own participation in auctions and continuous sessions, or in lit versus dark venues, therefore requiring a precise classification of intraday market data. 
