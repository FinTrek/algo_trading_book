...
\noindent\textbf{Neural Networks:} This is one of the main methods of machine learning where a performance of a task is learnt by analyzing training examples that have been hand-labeled in advance. The neural network in its simplest form can be represented as follows:
...
A more general form of neural net is given in Figure~\ref{fig:neural_net} below. Some practical issues with observing in fitting these models are given below. Because these models are over-parametrized, the optimization in \eqref{eqn:rby1it} can be a nonconvex function.
